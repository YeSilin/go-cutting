...
func main() {
	...
	power, tips = model.RestrictingSoftwareUse2(PrivateFile, 1.000097, tools.GetNtpTime(), 30) // 这里改版本信息！！！！！！！！！！！！！！！！！！！！
	// 如果权限不是true
	if !power {
		fmt.Println(tips)
		time.Sleep(5 * time.Second) // 休眠五秒
	...
	for {
		...
		color.LightCyan.Println("\n " + (strings.Repeat("-", 20)) + " Welcome to the GoCutting v1.0.97 app " + strings.Repeat("-", 20))
