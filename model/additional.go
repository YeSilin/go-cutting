...
func RunPSDRepairKit() {
	// 创建一个协程使用cmd启动外部程序
	dataPath := "data/software/PSDRepairKit/PSDRepairKit.exe"
...
func RunAdvancedExcelRepair() {
	// 创建一个协程使用cmd启动外部程序
	dataPath := "data/software/AdvancedExcelRepair/AER.exe"
	go exec.Command("cmd.exe", "/c", "start "+dataPath).Run()
	fmt.Println("\n:: 正在打开附加工具XLS文件修复，请稍后...")
}
...
func RunRevokeMsgPatcher() {
	// 创建一个协程使用cmd启动外部程序
	dataPath := "data/software/RevokeMsgPatcher.exe"
	go exec.Command("cmd.exe", "/c", "start "+dataPath).Run()
	fmt.Println("\n:: 正在打开附加工具微信QQ防撤回，请稍后...")
}
...
func RunW10DigitalActivation() {
	// 创建一个协程使用cmd启动外部程序
	dataPath := "data/software/W10DigitalActivation.exe /activate"
...
func ImportTakeOwnership() {
	// 创建一个协程使用cmd启动外部程序
	cmd := exec.Command("cmd.exe", "/c", "regedit /s .\\data\\regedit\\takeOwnership.reg")
	go cmd.Run()
...
func ImportNewTextFile() {
	// 创建一个协程使用cmd启动外部程序
	cmd := exec.Command("cmd.exe", "/c", "regedit /s .\\data\\regedit\\newTextFile.reg")
	go cmd.Run()
	fmt.Println("\n:: 右键菜单已添加，此项附加功能的命令需要右键管理员身份运行本软件方可生效！")
}
