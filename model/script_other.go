...
// CopyOriginalImageStamping 暗号-3，复制原图盖印
...
function copyOriginalImageStamping() {
    ...
    // 保存活动的历史记录状态，没有生成历史的操作保存不了
    var savedState = app.activeDocument.activeHistoryState;

    app.activeDocument.suspendHistory("复制原图盖印", "mergeHistory()");
...
// CopyOriginalImageGroup 暗号-4，复制原图编组
func CopyOriginalImageGroup() {
	// 切图文档的正则 ^\d{12}_[\u4e00-\u9fa5]{1,10}_\d{1,5}x\d{1,5}
...
//复制原图编组
function copyOriginalImageGroup() {
    // 打开的文档不够多直接返回
    if (app.documents.length < 2) {
        return;
    }
    ...
    // 合并成一条历史记录
    app.activeDocument.suspendHistory("复制原图编组", "mergeHistory(srcDoc, dstDoc)");
...
// ClearMetadata 暗号-7，深度情况文档，生成清除元数据第四版js，让文件更小巧，带进度条
func ClearMetadata() {
